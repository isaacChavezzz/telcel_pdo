...
$sentencia = $base_de_datos->query("SELECT tbl_ventas.total, tbl_ventas.fecha, tbl_ventas.id, GROUP_CONCAT(	tbl_productos.codigo, '..', tbl_productos.id_paqYrec, '..',  tbl_productos.paquetes_y_recargas, '..', tbl_productos.id_chip, '..',  tbl_productos.chip, '..', tbl_productos.id_accesorios, '..',  tbl_productos.accesorios, '..', tbl_productos.id_tel, '..',  tbl_productos.telefonos, '..', tbl_productosvendidos.cantidad SEPARATOR '__') AS tbl_productos FROM tbl_ventas INNER JOIN tbl_productosvendidos ON tbl_productosvendidos.id_venta = tbl_ventas.id INNER JOIN tbl_productos ON tbl_productos.id = tbl_productosvendidos.id_producto GROUP BY tbl_ventas.id ORDER BY tbl_ventas.id;");
